...
After every task is complete, a student can save a `*.wormie-submission.json` file outside the project. The submission contains finalized progress and only the evidence categories accepted at enrollment:
...
Submission hashes provide integrity checks, not authorship or non-repudiation. Local JSON transport intentionally does not provide remote authentication, automatic classroom synchronization, revocation, server-side retention, or teacher identity verification. Those require a hosted account service and signed server records; the local schemas and revision identifiers are the migration boundary for that future service.
